...
> 최종 업데이트: 2026-06-30 (Phase 3 완료, Phase 3-1 추가)
...
| Phase 3-1 | Shared Links DB 기록 저장 (F006) | 🔄 진행 예정 | 0% |
| Phase 4 | PDF 생성 기능 | ⏳ 예정 | 0% |
| Phase 5 | 배포 및 마무리 | ⏳ 예정 | 0% |

**전체 진행률: 약 50% (3 / 6 Phase 완료)**
...
- **Task 012: Shared Links DB 저장 함수 구현 및 링크 생성 API 연동** - 우선순위
  - [ ] `lib/notion.ts`에 `saveSharedLink` 함수 추가 (fetch REST API 기반)
  - [ ] `/api/links/route.ts`에서 토큰 발급 후 비차단 방식으로 `saveSharedLink` 호출
  - [ ] `AdminLinkCreateForm`에서 일지 제목을 API body에 포함하여 전송

- **Task 013: Shared Links 저장 E2E 테스트**
  - [ ] Playwright MCP로 어드민 전체 플로우 실행 (로그인 → 일지 선택 → 만료일 → 링크 생성)
  - [ ] Notion REST API로 Shared Links DB 조회하여 신규 행 추가 검증
  - **## 테스트 체크리스트**: 행의 이름·일지ID·만료일 속성 값이 입력값과 일치하는지 확인
