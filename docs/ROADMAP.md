...
- **Task 014: PDF 문서 컴포넌트 구현 (F004)** ✅ - 완료
  ...
  - [x] Noto Sans KR 한글 폰트 등록 (Google Fonts CDN woff2) 및 블록 타입별 PDF 요소 매핑
  - [x] 헤더(제목·날짜), 본문(블록 렌더링), 푸터(페이지 번호) 레이아웃 구성

- **Task 015: PDF 다운로드 버튼 연동 및 E2E 테스트** ✅ - 완료
  - [x] `PDFDownloadLink` 래퍼 컴포넌트 신규 생성 (`components/pdf/pdf-download-button.tsx`)
  - [x] `StudyLogViewer` 더미 버튼 제거, dynamic import + ssr:false로 교체
  - [x] 파일명 형식: `스터디일지_제목_날짜.pdf`, 로딩 상태 처리
