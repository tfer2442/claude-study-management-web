...
> **🧪 핵심 원칙**: API 연동이나 비즈니스 로직 구현과 같은 작업은 테스트를 꼼꼼히 수행할 수 있도록 작성하세요. 구현을 한 후에는 **반드시** 테스트를 수행해야 합니다. 테스트 시에는 **Playwright MCP**를 사용하세요. **단계별로 생각하며** 로드맵을 작성하세요.

### 🧠 단계별 사고 (Chain-of-Thought)

로드맵 작성 전, 다음 질문에 순서대로 답하며 생각하세요:

1. PRD에서 핵심 기능은 무엇인가?
2. 기능 간 의존성은 어떻게 되는가?
3. 어떤 순서로 개발해야 리스크가 최소화되는가?
4. 각 기능 구현 후 어떻게 테스트할 것인가? (Playwright MCP 시나리오)
5. 완료 기준은 측정 가능한가?
...
- 수락 기준과 완료 조건 정의
- **🚨 API 연동 및 비즈니스 로직 구현 시 Playwright MCP를 활용한 테스트 필수 — 구현 후 반드시 테스트 수행**
- 각 구현 단계 완료 후 Playwright MCP로 테스트 시나리오 실행 및 결과 검증
- 테스트 미통과 시 다음 단계로 진행 금지
